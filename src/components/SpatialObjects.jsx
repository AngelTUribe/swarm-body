...
const SpatialObjects = ({ handsPositionRef }) => {
  ...
  useFrame(() => {
    ...
    // 1. PHASE GATE: Hide everything during boot
    ...
    if (holeRef.current) {
      ...
      const targetColor = new THREE.Color(uiState.isSnapped ? '#ffffff' : '#00ffcc');
    ...
    Object.keys(uiState.projects).forEach((id) => {
      ...
      if (cube && pState) {
        ...
        cube.position.set(pos.x, pos.y, uiState.draggedId === id ? 0.5 : 0); 
        
        // SNAPPING LOGIC: If snapped, stop spinning
        if (uiState.draggedId === id && !uiState.isSnapped) {
           cube.rotation.x += 0.08; cube.rotation.y += 0.08;
           cube.scale.setScalar(0.7);
        } else {
           cube.rotation.set(0, 0, 0); // Lock rotation
