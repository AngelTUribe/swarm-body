...
const DJCenter = ({ handsPositionRef }) => {
    ...
    const djLoop = () => {
      ...
      if (hands[1]) {
        ...
        if (isPinching) {
          ...
          const scratchRate = (normalizedX * 4) - 1.0; 
          
          audioElRef.current.playbackRate = Math.max(0.1, scratchRate); 
  ...
  return (
    // ADDED pointerEvents: 'auto' to the main wrapper just to be safe
    <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: '#00ffcc', fontFamily: 'monospace', pointerEvents: 'auto' }}>
      
      {!audioLoaded ? (
        <div style={{ border: '2px dashed #00ffcc', padding: '40px', borderRadius: '16px', textAlign: 'center' }}>
          <h2>INITIALIZE AUDIO DECK</h2>
          {/* ADDED cursor, pointerEvents, and zIndex to force clickability */}
          <input 
            type="file" 
            accept="audio/*" 
            onChange={handleFileUpload} 
            style={{ marginTop: '20px', color: '#fff', cursor: 'pointer', pointerEvents: 'auto', position: 'relative', zIndex: 9999 }} 
          />
        </div>
      ) : (
        <div style={{ display: 'flex', width: '100%', height: '100%', padding: '20px', gap: '40px' }}>
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
             ...
             <button 
               onClick={togglePlay} 
               style={{ marginTop: '20px', padding: '10px 30px', backgroundColor: 'transparent', border: '2px solid #00ffcc', color: '#00ffcc', fontSize: '1.2rem', borderRadius: '8px', cursor: 'pointer', pointerEvents: 'auto', position: 'relative', zIndex: 9999 }}
             >
